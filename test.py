...
def cmd_list(args):
   if args.loads:
      ...
      for number in lj.loads():
         ...
         if args.hide_off and level == 0: continue
         name = lj.get_load_name(number)
         if level == 0: level_string = 'off'
         elif level == 99: level_string = 'on'
         else: level_string = 'at '+str(level)+'%'
   ...
   if args.buttons:
      ...
      for number in lj.button_switches():
         if args.hide_off and not switch_states[number]: continue
...
parser_switch_on = subparser_switch.add_parser('press', help='Simulate pressing a switch.')
parser_switch_on.set_defaults(func=cmd_switch_press)
parser_switch_off = subparser_switch.add_parser('release', help='Simulate releasing a switch.')
...
parser_monitor = subparsers.add_parser('monitor', help='Monitor items for state changes.')
